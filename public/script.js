...
function applyLanguage(lang) {
  ...
  document.querySelectorAll("[data-i18n]").forEach(el => {
    ...
    // Never replace a form label's contents: doing so would delete its input/textarea.
    if (el.tagName === "LABEL") {
      const target = el.querySelector(":scope > span");
      if (target) target.textContent = dict[key];
      else {
        const firstText = [...el.childNodes].find(n => n.nodeType === Node.TEXT_NODE);
        if (firstText) firstText.nodeValue = dict[key];
      }
      return;
    }
